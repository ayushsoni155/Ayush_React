...
const secretKey = '@@@@1234@bytewise24'; // Define the secret key for encryption
...
const BuyLabManual = () => {
  ...
  const [cart, setCart] = useState(() => {
    ...
    if (storedCart) {
      try {
        const bytes = CryptoJS.AES.decrypt(storedCart, secretKey);
  ...
  useEffect(() => {
    const encryptedCart = CryptoJS.AES.encrypt(
      JSON.stringify(cart.map(item => ({ subject_code: item.subject_code, quantity: item.quantity }))),
      secretKey
    ).toString();
  ...
  const addToCart = (manual) => {
    setCart((prevCart) => {
      ...
      if (existingItemIndex >= 0) {
        ...
        return [...prevCart, { subject_code: manual.subject_code, quantity: 1 ,product_name:"",sellingPrice:0}];
      }
    });
