...
const Header = () => {
  ...
  // Check signup status
  const isSignupDone = cookies.signupStatus
    ? decryptCookie(cookies.signupStatus)
    : null;
        ...
        try {
          const response = await fetch(
            `${process.env.REACT_APP_API_URL}/verify-user`, // Backend endpoint
            {
              ...
              body: JSON.stringify({ userData: loginData }), // Send decrypted user data
            }
          );
          ...
          if (response.ok) {
            ...
            if (!result.valid) {
              // If data is invalid, logout the user
